...
| 13 | Edit expense | [EditExpense.js](../client/src/components/EditExpense.js) | [Expenses.js](../client/src/pages/Expenses.js) | `PATCH /expense/updateExpense/:id` | Built |
...
[EditPasswordForm.js](../client/src/components/EditPasswordForm.js) → `PATCH /users/:id/editPassword` ([userRoutes.js:80](../server/routes/userRoutes.js#L80)). One of the two forms that hold their own state and send their own request rather than taking them from a page; the VAT calculator is the other.

The route had been moved to `/editPassword/:id` without the form, so every submission reached the 404 fallback rather than the handler. The route is now back on `/:id/editPassword`, the shape its own comment block, its log messages and the sibling `/:id/editUser` are all written against.

| Field | Control | Required | Constraints | Notes |
|---|---|---|---|---|
| `currentPassword` | `password` | Yes | — | Verified against the stored hash by the route. `autoComplete='current-password'` |
| `newPassword` | `password` | Yes | min 8 chars, max 1024, at least one special character, must differ from `currentPassword` | `strongPasswordRegex`, the same pattern `checkPassword` applies on the server. `autoComplete='new-password'` |
| `confirmNewPassword` | `password` | Yes | Must equal `newPassword` | Never sent; it exists only to catch a typo |

- All three component rules return before `setLoading`, so a validation failure does not leave the button disabled.
- The "must differ from the current password" rule mirrors the 400 the route returns, so the reuse case is caught without spending a request.
- Failures are reported inline through `formError` (`role='alert'`) as well as through `setError`, so the reason stays next to the submit button. The per-field strength, match and reuse messages are derived from what has been typed and each is tied to its input by `aria-describedby`.
- The user id comes from `currentUser.userId`: the API shapes a user with `toPublicJSON`, so `currentUser.id` is always undefined.
- Each field has its own show/hide toggle. On success, and on *Clear Form*, `resetForm` empties all three fields and closes all three toggles, so a password is never left visible in a field the user believes they have emptied.
...
- Writing is now built on both sides too. `PATCH /expense/updateExpense/:id` ([expenseRoutes.js:645](../server/routes/expenseRoutes.js#L645)) edits one expense: it is partial, so a field the body does not carry is left as it is stored, and changing the trip **moves** the expense — it is pushed onto the destination budget with its own `_id` kept and pulled off the one it came from, because an expense is embedded in a budget rather than pointing at one. `DELETE /expense/delete/:id` ([expenseRoutes.js:863](../server/routes/expenseRoutes.js#L863)) pulls one expense off its parent and leaves the budget standing, unlike `DELETE /budget/deleteBudget/:id`, which takes every expense embedded in a budget with it.
- [ExpensesList.js](../client/src/components/ExpensesList.js) is built: a table of every expense with a details panel resolved out of that list with `useMemo`, the way [EntriesList.js](../client/src/components/EntriesList.js) resolves its own — no read of its own is needed, because `fetchExpenses` returns every field the panel reports on. Its EDIT calls `startExpenseEdit`, which reads the expense back by its id through `fetchExpense` so the form opens against what is currently stored rather than against the copy the list is holding, which may have moved since it was loaded. Its DELETE confirms first, naming the expense, its figure and the trip it was filed against, then reports whether the expense actually went so the panel is only closed once the API has answered.
...
| [EditExpense.js](../client/src/components/EditExpense.js) | An expense, section 10 | `PATCH /expense/updateExpense/:id`, written | [EditExpense.css](../client/src/css/componentCss/EditExpense.css) |
...
| Where | Issue |
|---|---|
| [app.js:65-73](../server/app.js#L65-L73) | Eight routers are mounted, including `/api` and `/budget`. Only `/export` has no router to mount |
| [expenseRoutes.js](../server/routes/expenseRoutes.js) | Complete: `/fetchBudgets`, `/fetchExpenses`, `/fetchExpense/:id`, `/addExpense`, `PATCH /updateExpense/:id` and `DELETE /delete/:id` are all written, so an expense can be read, added, changed, moved to another trip's budget and removed |
...
| [exportRoutes.js](../server/routes/exportRoutes.js) | A header comment listing three `GET` endpoints and no router, so it is not required or mounted by [app.js](../server/app.js) and has no path even once it is written |
...
| [Expenses.js:95](../client/src/pages/Expenses.js#L95) | `currencyOptions` is initialised from `FALLBACK_CURRENCIES` and never refetched — a `useState` with no setter rather than a deliberate fallback. Now that `/api/currencies` answers, the add expense form is the only currency select still offering the offline snapshot instead of the provider list; [Budget.js](../client/src/pages/Budget.js#L59) shows the pattern to copy |
| [EditPasswordForm.js](../client/src/components/EditPasswordForm.js) | Failures are now reported inline with `role='alert'` and `aria-invalid`, as in [EditUserForm.js](../client/src/components/EditUserForm.js), but the field-level messages are derived from the current value rather than gated on a `touched` state, so a message appears while the field is still being typed into. Success is still reported with `window.alert`, for the reason in the row below |
