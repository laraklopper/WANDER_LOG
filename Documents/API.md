...
| `PUT` | [`/api/updateConversion/:id`](../server/routes/apiRoutes.js#L268) | JWT | Implemented | Reprices one saved conversion at today's rate, and can move it onto another target currency |
| `DELETE` | [`/api/history/:id`](../server/routes/apiRoutes.js#L381) | JWT | Implemented | Removes one of the user's saved conversions |
...
Every route on this router is called from the client, all six through [Budget.js](../client/src/pages/Budget.js), which owns the converter's state and passes the requests down: `/api/currencies`, `/api/convert` and `/api/save` to [CurrencyConverter.js](../client/src/components/CurrencyConverter.js), `/api/history` and `/api/history/:id` to [ConversionsList.js](../client/src/components/ConversionsList.js), and `/api/updateConversion/:id` — with a second call to `/api/convert` — to [UpdateConversion.js](../client/src/components/UpdateConversion.js). Nothing on the list is repriced — each record holds the rate its save fetched, so the display formatters in [currencyFunc.js](../client/src/util/currencyFunc.js#L61-L104) only read stored figures. `PUT /api/updateConversion/:id` is the one route that moves a stored rate, and only when the user asks it to.
...
| `PUT /api/updateConversion/:id` | `:id`, Body: `from`, `to`, `amount` | A **PUT** rather than a `PATCH`: a conversion is four figures and three of them are the input the fourth was worked out from, so the whole record is replaced — a body that changed the target currency alone would leave a rate stored for a pair it was never quoted for. The body goes through the same `parseConversionInput` as `POST /api/save`, and the rate is **fetched here** for the same reason, so a conversion cannot be updated into something a new one could not be saved as. Matched on the id and the user together, so another account's record is a `404`. `400` on a malformed id or an unusable figure, `502` when Frankfurter cannot price the pair — answered before anything is written, so the saved conversion is left as it was. Returns `{ success, message, updated, previous, rateChanged }`: `previous` is what the record held before the call, which is gone from the database once it answers, and `rateChanged` is computed server-side because a changed target currency prices a different pair and so is not the same rate moving |
| `DELETE /api/history/:id` | `:id` | Matched on the id and the user in a single query, same as `/vat/history/:id`. `400` on a malformed id, `404` when not found. Returns `conversionId` so the client can drop the row, though `deleteConversion` in [Budget.js](../client/src/pages/Budget.js#L232) refetches the list instead, so what is on screen is what the database holds |
